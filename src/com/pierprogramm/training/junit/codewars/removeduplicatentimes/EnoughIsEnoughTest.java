package com.pierprogramm.training.junit.codewars.removeduplicatentimes;

import org.junit.Test;

import static org.junit.Assert.*;

public class EnoughIsEnoughTest {

    @Test
    public void testDeleteNth() throws Exception {
//        assertEquals("1", new int[]{20,37,21}, EnoughIsEnough.deleteNth(new int[]{20, 37, 20, 21}, 1));
//        assertEquals("2", new int[]{1, 1, 3, 3, 7, 2, 2, 2}, EnoughIsEnough.deleteNth(new int[]{1,1,3,3,7,2,2,2,2}, 3));
//        assertEquals("3", new int[]{1,2,3,1,2}, EnoughIsEnough.deleteNth(new int[]{1, 2, 3, 1, 2, 1, 2, 3}, 2));


        assertArrayEquals("1",
                new int[] { 20, 37, 21 },
                EnoughIsEnough.deleteNth( new int[] { 20, 37, 20, 21 }, 1 )
        );
        assertArrayEquals("2",
                new int[] { 1, 1, 3, 3, 7, 2, 2, 2 },
                EnoughIsEnough.deleteNth( new int[] { 1, 1, 3, 3, 7, 2, 2, 2, 2 }, 3 )

        );
        assertArrayEquals("3",
                new int[] { 1, 2, 3, 1, 1, 2, 2, 3, 3, 4, 5 },
                EnoughIsEnough.deleteNth( new int[] { 1, 2, 3, 1, 1, 2, 1, 2, 3, 3, 2, 4, 5, 3, 1 }, 3 )
        );
        assertArrayEquals("4",
                new int[] { 1, 1, 1, 1, 1 },
                EnoughIsEnough.deleteNth( new int[] { 1, 1, 1, 1, 1 }, 5 )
        );
        assertArrayEquals("5",
                new int[] { },
                EnoughIsEnough.deleteNth( new int[] { }, 5 )
        );

    }
}